class QHelper {
    ...
    sumarTabla() {
        ...
        $("table tr td").each(function() {
            sumsRows[Math.floor(c / 3)] += new Number(this.innerHTML);
            sumsCols[Math.floor(c % 3)] += new Number(this.innerHTML);
            total += new Number(this.innerHTML);
            c++;
        });
        var i = -1;
        $("table tr").each(function() {
            if (i >= 0) {
                $(this).prepend($("<td></td>").text(sumsRows[i]));
            } else {
                $(this).prepend($("<th></th>").text("suma"));
            }
            i++;
        });
        $("tbody").prepend("<tr>\n\t<th>suma</th>\n\t<td>," + sumsCols[0] + "</td>\n\t<td>," + sumsCols[1] + "</td>\n\t<td>," + sumsCols[2] + "</td>\n\t<td>," + total + "</td></tr>");
